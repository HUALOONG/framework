 ...
 * 钉钉自定义机器人消息发送器（具体实现）。
 *
 * <p>基于 JDK 内置 {@link HttpClient}，按钉钉机器人 webhook 协议发送 Markdown 消息。
 * 零外部依赖，可直接实例化使用，也可注入自定义 {@link HttpClient} 以复用连接池或便于测试。</p>
 *
 * <p>钉钉机器人 Markdown 消息格式：
 * <pre>
 * {
 *   "msgtype": "markdown",
 ...
 */
@NullMarked
public class DingTalkWebhookMessageSender extends DingTalkMessageSender {
     ...
     */
    public DingTalkWebhookMessageSender() {
     ...
     */
    public DingTalkWebhookMessageSender(HttpClient httpClient) {
     ...
     */
    public DingTalkWebhookMessageSender(HttpClient httpClient, Duration timeout) {
    ...
    protected void doSend(String webhook, String title, String content) {
        Assert.hasText(webhook, "钉钉 webhook 地址不能为空");
        String body = dingTalkMarkdownJson(title, content);

        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(webhook))
                .timeout(timeout)
                .header("Content-Type", "application/json; charset=UTF-8")
                .header("User-Agent", "Jowen-Framework-DingTalkSender")
        ...
        try {
            ...
            throw new ExtrasException("钉钉消息发送失败: " + webhook, e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new ExtrasException("钉钉消息发送被中断: " + webhook, e);
        }

        int status = response.statusCode();
        if (status < 200 || status > 299) {
            throw new ExtrasException(ErrorCodeEnum.INTERNAL_ERROR,
                    "钉钉机器人响应异常，状态码=" + status + ", webhook=" + webhook);
        }
    }

    /**
     * 构造钉钉机器人 Markdown 消息 JSON 体。
     *
     * @param title   标题
     * @param content 内容
     * @return JSON 字符串
     */
    private static String dingTalkMarkdownJson(String title, String content) {
        return "{\"msgtype\":\"markdown\","
                + "\"markdown\":{\"title\":" + escapeJson(title)
