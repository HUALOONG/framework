...
### 注释规范
- **参数校验**：所有 `public` 方法必须在方法体第一行对入参进行非空或合法性校验（如 `Assert.notNull`），并抛出明确的异常信息。 
- **Javadoc 完整性**：`public` 方法的 Javadoc 必须包含 `@param` 和 `@return` 标签，简要说明参数约束和返回值含义。
- **复杂逻辑说明**：对于复杂的业务逻辑、算法实现或临时的修复方案（Hack/Workaround），必须使用 `//` 添加行内注释，解释“为什么”这样写。
- **禁止废话注释**：禁止写显而易见的注释（如 `i++ // i加1`），注释应提供代码之外的上下文信息。
